...
import { UALength, userAgents } from "./userAgent";
...
export const allStatus = ["all", "completed", "ongoing"];
...
class ComicsApi {
  ...
  constructor() {
    ...
    this.siteUrl = `${process.env.SITE_URL}/api`;
  }
  ...
  private async getComics(path: string, page: number = 1, statusKey: Status = "all"): Promise<any> {
    ...
    try {
      const [$, allGenres] = await Promise.all([
        this.createRequest(`${path + (path.includes("?") ? "&" : "?")}status=${status[statusKey]}&page=${page}`),
        this.getGenres(),
      ]);
      const total_pages =
        $(".pagination li:nth-last-child(2) a").attr("href")?.split("=").at(-1) || $(".pagination .active").text() || 1;
      if (page > total_pages) {
        return { status: 404, message: "Page not found" };
      }
      const comics: any = Array.from($(".ModuleContent .items .item")).map((item) => {
        const thumbnail = $(".image img", item).attr("src");
  ...
  public async getRecentUpdateComics(page: number = 1, status: Status): Promise<any> {
    try {
  ...
  public async getComicsByGenre(genreId: string, page: number = 1, status: Status): Promise<any> {
    try {
  ...
  public async getChapter(comicId: string, chapterId: number): Promise<any> {
    try {
      ...
      const images = Array.from($(".page-chapter img")).map((img, idx) => {
        const src = `${this.siteUrl}/images?src=${$(img).attr("data-sv1")}`;
        const backup_src = `${this.siteUrl}/images?src=${$(img).attr("data-sv2")}`;
        return { page: idx + 1, src, backup_src };
      });
      const [comic_name, chapter_name]: any = this.trim($(".txt-primary").text().trim())?.split(" - ");
  ...
  public async getSearchSuggest(query: string): Promise<any> {
    try {
      ...
      const { data } = await axios.get(`https:${this.hosts[0]}/Comic/Services/SuggestSearch.ashx?q=${query}`, {
      ...
      const suggestions = Array.from($("li")).map((comic) => {
        ...
        return {
          id,
          title,
          thumbnail: `https://${this.hosts[0]}/public/images/comics/${this.getId(thumbnail)}`,
          lastest_chapter: lastest_chapter.startsWith("Chapter") ? lastest_chapter : "Updating",
  ...
  public async searchComics(query: string, page: number = 1): Promise<any> {
    try {
      return await this.getComics(`tim-truyen?keyword=${query.trim().replace(/\s+/g, "+")}`, page);
    } catch (err) {
      throw err;
    }
  }
